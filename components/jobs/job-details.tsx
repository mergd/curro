...
export function JobDetails({ id }: { id: Id<"jobs"> }) {
  ...
  return (
    <div>
      ...
      <p className="text-lg text-gray-700">{job.company?.name} - {job.location}</p>
      ...
      {job.company && (
        <div className="mt-8 p-4 border rounded-lg">
          ...
          <p><strong>Stage:</strong> {job.company.stage}</p>
          <p><strong>Employees:</strong> {job.company.numberOfEmployees}</p>
          {job.company.recentFinancing && (
            <p>
              <strong>Recent Financing:</strong> {job.company.recentFinancing.amount} on {job.company.recentFinancing.date}
            </p>
          )}
          {job.company.investors && (
            <p><strong>Investors:</strong> {job.company.investors.join(", ")}</p>
          )}
        </div>
      )}
    </div>
